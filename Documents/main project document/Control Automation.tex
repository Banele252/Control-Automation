...
\begin{titlepage}
    ...
    \begin{tabular}{|r|l|l|p{4cm}|l|}
        ...
        \hline
          1.0.0 & Banele Mdluli & Pending & compiling documentation & 02 February 2026\\
        \hline
          2.0.0 & Banele Mdluli & Approved & Replaced Supbase with Mongo DB & 23 February 2026\\
        \hline
          2.1.0 & Banele Mdluli & Approved & Added agent control performance summary & 5 March 2026\\
        \hline
    \end{tabular}
...
\subsection{Fast API}
\subsubsection{Mongo DB Data integration}
To imitate real world third party tools, Mongo DB is used as a third party data storage. The Mongo DB
...
\begin{itemize}
    ...
    \end{itemize}
    \item Create a Mongo DB project and retrieve the database API.
    \item Create four collections in Mongo DB.
    \begin{itemize}
    ...
    \end{itemize}
    \item Create a FAST API that transfers A.I generated data to the Mongo DB table.
...
linked to the Mongo database. ADF adheres to the medallion architecture, which consists of three layers Bronze, Silve and Gold. Bronze is associated
...
\begin{itemize}
    \item Create a pipeline that copies the information from Mongo database to a Azure blob storage.
    \item Move the data from the Azure blob storage to a Azure datalake storage Gen2 that has hierarchical features .
...
\begin{itemize}
    ...
    \item  Push the silver container table (control performance outcome) to a postgres table
\end{itemize}

\subsection{AI agent and machine learning model}
There are two use cases that will require A.I agents. The first use case is about an A.I agent that creates a summary 
based on control performance outcome. The second use case is about an A.I agent that handles user interaction in the platform and provides 
the desired outcome.\\
\subsubsection{Use case 1: Control performance summary agent}
The agent will extract a control daily exception outcomes. There after perform a summary based on a list of requirements. Not all requirement can
be fulfilled for every control. However, they will be mandatory and optional requirements. The control summary will be divided into two levels, namely,
first and second summary. The first level is compulsory for every control and the second level will depend on the type of control.  
\begin{itemize}
    \item Create an agent that will review control outcome exceptions and provide a first level summary.
    \item The summary should contain the following:
    \begin{itemize}
        \item Compulsory summary should have the following: 
        \begin{itemize}
            \item The number of records.
            \item The total usage.
            \item The total spend. 
        \end{itemize}
        \item Optional summary (in addition to compulsory):
        \begin{itemize}
            \item Top five accounts based on usage, spend and record frequency.
            \item Five accounts that have appeared in two previous days reviews.
            \item Trend explaination since the start of the month (are the number exceptions increasing per day, the month to date trend).
        \end{itemize}
    \end{itemize}
    \item Store the review outcome in another postgres table called daily control outcome review.
\end{itemize}

\subsubsection{Use case 2: User interaction agent}
...
going to be functionalised for the agent to call if needed. The AI design pattern (workflow) used is Routing. Routing is a design pattern 
where an LLM decides. which child LLM or process is executed. There is no aggregation or synthesis used for the final outcome.
...
\begin{itemize}
    ...
    \item One of the child agents must call the postgres database using API (developed using Fast API).
    \item Functionalise an API call to the ML model inference. 
    \item Each agent should have guardrails.
    \item Process the users inputs and provide the best outcome based on the information available.
\end{itemize}
